...
final class SusuServiceSusuNextTransactionsRequest
{
    ...
    public function execute(Customer $customer, string $susu_resource, int $page): array
    {
        return Http::withHeaders(['Content-Type' => 'application/vnd.api+json', 'Accept' => 'application/vnd.api+json'])
            ->get(url: $this->service->base_url.'customers/'.$customer->resource_id.'/susus/'.$susu_resource.'/transactions?size=3&page='.$page)
            ->json();
    }
}
