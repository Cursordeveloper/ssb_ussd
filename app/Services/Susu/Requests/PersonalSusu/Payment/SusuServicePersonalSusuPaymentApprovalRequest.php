...
final class SusuServicePersonalSusuPaymentApprovalRequest
{
    ...
    public function execute(Customer $customer, array $data, string $susu_resource, string $payment_resource): array
    {
        return Http::withHeaders(['Content-Type' => 'application/vnd.api+json', 'Accept' => 'application/vnd.api+json'])
            ->post(url: $this->service->base_url.'customers/'.$customer->resource_id.'/personal-susus/'.$susu_resource.'/payments/'.$payment_resource.'/approvals', data: $data)
            ->json();
    }
}
