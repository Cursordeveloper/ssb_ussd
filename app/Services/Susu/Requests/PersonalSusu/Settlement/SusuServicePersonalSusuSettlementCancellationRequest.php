...
final class SusuServicePersonalSusuSettlementCancellationRequest
{
    ...
    public function execute(Customer $customer, array $data, string $susu_resource, string $settlement_resource): array
    {
        return Http::withHeaders(['Content-Type' => 'application/vnd.api+json', 'Accept' => 'application/vnd.api+json'])
            ->post(url: $this->service->base_url.'customers/'.$customer->resource_id.'/personal-susus/'.$susu_resource.'/settlements/'.$settlement_resource.'/cancellations', data: $data)
            ->json();
    }
}
