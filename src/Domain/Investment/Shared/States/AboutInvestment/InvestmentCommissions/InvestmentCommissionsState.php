...
namespace Domain\Investment\Shared\States\AboutInvestment\InvestmentCommissions;
...
final class InvestmentCommissionsState
{
    public static function execute(Session $session, $service_data): JsonResponse
    {
        // Return to the AboutSusuState if user input is (0)
        if ($service_data->user_input === '0') {
            return BackToAboutInvestmentAction::execute(session: $session, service_data: $service_data);
        }
        ...
        if ($service_data->user_input === '#') {
            ...
            return InvestmentCommissionsMenu::nextContentMenu(session: $session);
        }

        // Return the invalidChoiceMenu
        return InvestmentCommissionsMenu::invalidChoiceMenu(session: $session);
    }
}
