...
final class FlexySusuCreateApprovalAction
{
    ...
    {
        // Execute the FlexySusuApprovalRequest and return the response data
        $response = (new FlexySusuApprovalRequest)->execute(
    ...
    {
        // Execute the FlexySusuCancellationRequest HTTP request
        (new FlexySusuCancellationRequest)->execute(
            customer: $session->customer,
            data: FlexySusuCancellationData::toArray(),
            susu_resource: $session->userInputs()['susu_resource'],
        );
